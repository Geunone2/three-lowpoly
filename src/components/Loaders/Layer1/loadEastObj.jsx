...
import groundLayout from "../maps/east/Ground.json";
...
export async function loadEastModels(scene, manager) {

    const loadModel = createObjLoader({
        manager,
        cache: eastModelCache,
    })
    ...
    for (const item of resolvedLayout) {
        const original = eastModelCache[item.type];
